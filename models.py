...
# Fix for entmax and ENTMAX_ALPHA not defined
try:
    from entmax import entmax
except ImportError:
    # fallback: define a dummy entmax that raises error if used
    def entmax(*args, **kwargs):
        raise ImportError("entmax is not installed. Please install with 'pip install entmax'.")

# Default value for entmax alpha (1.5 is standard for entmax15)
ENTMAX_ALPHA = 1.5
...
def get_model(args, training=False):
    ...
    #     import wandb  # Removed because wandb is not installed and not required
