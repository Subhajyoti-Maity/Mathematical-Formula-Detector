...
def predict_formulas(img, model):
    ...
    pred = non_max_suppression(pred, 0.12, 0.45)
    ...
    for i, det in enumerate(pred):  # detections per image
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                # Lower confidence threshold and remove class filtering to catch all formulas
                if conf < 0.12:
                    continue
                ...
                # Filter out only very small regions (likely noise, not formulas)
                if height < 10:
                    continue
                bboxes.append([x1, y1, x2, y2, conf.item(), cls.item()])
